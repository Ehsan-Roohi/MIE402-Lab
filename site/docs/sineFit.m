function SineParams=sineFit(x,y,Plot)
...
%              Plot: optional, plot graphics if ommited. Do not plot if Plot=0.
...
%%Check for monospaced x-values
xDiff=diff(x);
xDiffMin=min(xDiff);
if max(xDiff)-xDiffMin>(x(2)-x(1))/100
  %Not evenly spaced by more than 1%, interpolate
  xDiffMean=mean(xDiff);
  dx=max(xDiffMean/4,xDiffMin);
  n=round((x(end)-x(1))/dx);
  xFFT=linspace(x(1),x(end)-eps,n);
  yFFT=interp1(x,y,xFFT,'linear');
else
  xFFT=x;
  yFFT=y;
end
...
FFTpeaksIndx = find(diff(sign(diff(P1)))<0)+1;%Find index of maximum peaks of FFT
FFTpeaksIndx2 = find(P1(FFTpeaksIndx)>0.8*maxFFT);
FFTpeaksIndx=FFTpeaksIndx(FFTpeaksIndx2);
...
elseif numel(FFTpeaksIndx)>1
  %Several peak values close to max peak value 
  %Improves number of good results by ~0.1%
  fIndxExtra=FFTpeaksIndx;
  AExtra=maxFFT;
  Numf=numel(FFTpeaksIndx);%number of frequencies
